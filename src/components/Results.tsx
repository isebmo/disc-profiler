    ...
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div ref={pdfRef}>
        ...
        <div className="profile-card mb-6 overflow-hidden" style={{ boxShadow: 'var(--shadow-ds-lg)' }}>
          <div className="profile-card__blob-header" style={{ background: dominantProfile.colorSurface }}>
            <svg viewBox="0 0 200 200">
              <path d={dominantProfile.blobPath} transform="translate(100 100)" fill={dominantProfile.color} opacity="0.3" />
            </svg>
            <span className="profile-card__icon" style={{ color: dominantProfile.textOnColor }}>
              {dominantProfile.dimension}
            </span>
          </div>
          <div className="profile-card__body">
            <span className="profile-card__tag" style={{ background: dominantProfile.colorLight, color: dominantProfile.colorDark }}>
              {t.results.dominant}
            </span>
            ...
            <p className="text-[#5F6368] dark:text-[#BDC1C6] leading-relaxed">{dominantText.description}</p>
          </div>
        </div>
        ...
        {secondaryProfile && secondaryText && (
          <div className="profile-card mb-8 overflow-hidden" style={{ boxShadow: 'var(--shadow-ds-md)' }}>
            <div className="profile-card__blob-header" style={{ background: secondaryProfile.colorSurface, height: 100 }}>
              <svg viewBox="0 0 200 200" style={{ width: 160, height: 160 }}>
                <path d={secondaryProfile.blobPath} transform="translate(100 100)" fill={secondaryProfile.color} opacity="0.3" />
              </svg>
              <span className="profile-card__icon text-[2rem]" style={{ color: secondaryProfile.textOnColor }}>
                {secondaryProfile.dimension}
              </span>
            </div>
            <div className="profile-card__body">
              <span className="profile-card__tag" style={{ background: secondaryProfile.colorLight, color: secondaryProfile.colorDark }}>
                {t.results.secondary}
              </span>
              <h3 className="font-display font-bold text-lg sm:text-xl text-[#202124] dark:text-white mb-2 tracking-tight">
                {secondaryText.title} — {secondaryText.name}
              </h3>
              <p className="text-[#5F6368] dark:text-[#BDC1C6] leading-relaxed">{secondaryText.description}</p>
            </div>
          </div>
        )}

        {/* Strengths as chips */}
        <Section title={t.results.strengths} icon={<StrengthIcon />} color={dominantProfile.color}>
          <div className="flex flex-wrap gap-2">
            {dominantText.strengths.map((s, i) => (
        ...
        {/* Watchouts */}
        <Section title={t.results.watchouts} icon={<WatchoutIcon />} color="#E37400">
          <ul className="space-y-3">
            {dominantText.watchouts.map((item, i) => (
        ...
        {/* Communication */}
        <Section title={t.results.communication} icon={<CommIcon />} color="#4285F4">
          <ul className="space-y-3">
            {dominantText.communication.map((item, i) => (
              <li key={i} className="flex items-start gap-3 text-[#3C4043] dark:text-[#BDC1C6]">
                <span className="mt-1.5 w-2 h-2 rounded-full flex-shrink-0" style={{ background: dominantProfile.color }} />
                {item}
              </li>
            ))}
          </ul>
...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; color: string; children: React.ReactNode }) {
  return (
